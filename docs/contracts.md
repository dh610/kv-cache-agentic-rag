...
PR #23부터 주장별로 검증 실패를 격리합니다. 실패·누락·주장 계약 오류가 있는 claim은 `unverified`로 옮기고, 살아남은 claim의 확인된 출처로 각 assessment와 TRL을 다시 검사합니다. 같은 기준에도 충분한 검증 근거가 남으면 판정을 유지하고, 그렇지 않으면 `확인 불가`로 보류합니다. 계약 오류는 이력과 needs_revision에 남기며 실행 실패는 failed를 유지합니다.
...
- 인용 Judge는 주장 하나와 그 주장이 인용한 Evidence만 받습니다. 모델에는 E1..En 별칭을 보여주고 실제 ID로 복원하며 claim ID는 호출 대상에 고정합니다. 웹 Evidence 자체는 PR #23의 원문 정제·질문 관련 발췌 결과입니다. 생성 모델의 요약문을 원문 근거로 대체하지 않습니다.
...
- scope를 지원하는 검색 어댑터는 각 시도에서 선정 기술(target)과 접근 전반(context)을 검색합니다. 예산은 질문별 최대 3시도이며 두 층은 별도 SearchRecord를 남깁니다. 기존 `search(question, attempt)` 어댑터는 단일 층으로 호환하고, 새 어댑터는 선택 인수 `scope="target"`을 지원합니다. 검색 동시 실행은 노드당 최대 4개입니다.
- 웹 scope는 검색 층을 기록하며 페이지 내용의 정확성이나 직접 채택을 자동 증명하지 않습니다. 선정 기술 자체의 채택과 TRL에는 target 근거만 허용하고, 시장·이해관계자에서 접근 전반의 평가를 별도로 표시합니다.
- 결과의 부재 진술은 claims 대신 unverified로 이동합니다. 확인된 인용이 있는 assessment/TRL에서는 확인되지 않은 알려진 출처를 제거하되, 존재하지 않는 ID는 계약 오류로 남깁니다.
- 표지·실제 페이지 번호 목차·문서별 인용 번호·제출 파일명은 정재웅님 PR 구성을 사용합니다. 한글 글꼴은 PDF에 내장합니다. 재조판은 `uv run python -m app.rerender_report --from outputs/local/<run-id> --out outputs/local/<new-layout>`로 실행하며 LLM을 호출하지 않습니다.
- 최종 산출물의 출처와 통합 범위는 [최종본 기준](final-report-baseline.md)을 참고하세요. `--first-pass`는 빠른 초안 옵션이며 선정된 최종 PDF를 재현하는 실행 모드가 아닙니다.
