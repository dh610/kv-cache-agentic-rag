...
전체 파이프라인은 종합 뒤 gaps가 남으면 해당 역할을 최대 1라운드 보완 재실행합니다(`limits.supplement=1`, 사용자 승인). 기술 조사를 재실행하면 의존 평가도 재실행합니다. 재실행되는 노드는 새 실행의 질문별 검색 예산을 사용하므로 추가 API 호출이 생깁니다. 개별 run_node에는 이 메인 그래프 루프가 없고 mock에서는 건너뜁니다. 0으로 설정하면 끌 수 있습니다.
