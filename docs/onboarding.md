...
전체 파이프라인은 종합 뒤 gaps가 남으면 해당 역할을 최대 1라운드 보완 재실행합니다(`limits.supplement=1`, 사용자 승인). 보완은 기존 근거·질문별 충분성·검색 이력을 재사용하고 부족한 질문만 새 검색 예산(최대 3회)으로 조회합니다. 기술 결과가 실제로 바뀐 경우 의존 평가도 재검증하며, 변경된 기술의 충분성부터 다시 확인합니다. 노드 안의 재검색 역시 충분하고 긍정·비판 검색을 마친 질문은 제외합니다. 전체 질문의 생성·인용 검증은 계속 수행합니다. 개별 run_node에는 이 메인 그래프 루프가 없고 mock에서는 건너뜁니다. 0으로 설정하면 끌 수 있습니다.
