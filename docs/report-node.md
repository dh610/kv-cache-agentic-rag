...
| 4.1~4.3 관점별 평가 | 서술 + 평가 노드 요약 + 검증된 근거 문장 + 절 끝 두 기술 비교표 | LLM claims `market/stakeholder/domain` + 코드 표 | 각 `*_result` |
...
| REFERENCE | 실제 인용 자료만, 가이드 표기 형식(논문·특허·웹), 문서별 1항목 | 코드 | `sources` |
...
- REFERENCE 형식(가이드 E·설계서 표 18 그대로): 논문 `저자(YYYY). 논문제목. 학술지/학회명, 권(호), 페이지.`, 특허 `출원인(YYYY-MM). 특허명, 특허번호/공개번호, URL`, 웹 `기관명 또는 작성자(YYYY-MM-DD). 제목. 사이트명, URL`. 권(호)·페이지(또는 arXiv 번호)와 특허번호는 `Evidence.citation_id`에 담으며, 논문은 `data/documents.yaml`의 `citation_id`에서 옵니다. 누락은 인수 검사가 보고합니다. 같은 문서의 여러 chunk는 하나의 항목으로 묶고 논문은 원문 URL, 인용 ID·페이지를 뒤에 남깁니다.
- 비교표 위치: 설계서 E.1대로 3장과 4장 각 절의 마지막 요소가 두 기술 비교표이고, 5.1은 관점×기술 교차표입니다.
...
- 설계서 해석 차이 명시: 시장의 근거 있는 등급 충돌에는 낮은 등급을, 종합의 근거 부족에는 확인 불가·판정 유보를 적용한다는 역할별 차이를 6.2에 적습니다. [최종 설계 점검](final-design-review.md)의 쟁점을 숨기지 않습니다.
